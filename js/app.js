...
function formatarMoeda(valor) {
  return toNumber(valor, 0).toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
}
...
function formatarCompetenciaLabel(competencia) {
  const mapa = { "01":"Jan","02":"Fev","03":"Mar","04":"Abr","05":"Mai","06":"Jun",
...
function validarSupabasePronto() {
  if (!window.supabaseClient) throw new Error("Supabase não configurado. Verifique js/supabase-config.js.");
...
function getCidadeAtual() { return byId("filtroCidade")?.value || ""; }
function getDoutorAtual()  { return byId("filtroDoutor")?.value  || ""; }
...
function somarUtilizadoPorNomeNosRegistros(competencia) {
  ...
  for (const item of registros) {
    const nome = normalizarNome(
      item.doutor_final || item.doutor || item.nome_doutor ||
      item.responsavel_fiscal || item.responsavel_fiscal_lido || ""
    );
    if (!nome) continue;
...
async function garantirDoutorNoSupabase(idOriginal, payloadBase) {
  ...
  const { data: existente, error: errorBusca } = await client
    .from("doutores_config").select("id, nome, nome_normalizado")
    .eq("nome_normalizado", payloadBase.nome_normalizado).maybeSingle();
  if (errorBusca) throw errorBusca;
  if (existente?.id) return existente.id;
  const { data: novo, error: errorInsert } = await client
    .from("doutores_config").insert(payloadBase).select("id").single();
...
async function sincronizarSaldosAdminNoDashboard() {
  ...
  try {
    ...
    const { data: doutoresSupabase, error: errorDoutores } = await client
      ...
      .select("id, nome, nome_normalizado, credito, pix_key, ativo, updated_by_email, updated_by_nome, updated_at")
      .order("nome", { ascending: true });
    if (errorDoutores) { console.warn("Não foi possível carregar doutores da ADM.", errorDoutores); return; }

    const meses = dashboardData?.meses_disponiveis || [];
    if (!meses.length) return;

    const { data: saldosSupabase, error: errorSaldos } = await client
      .from("doutores_saldos_mensais").select("*").in("competencia", meses);
    if (errorSaldos) { console.warn("Não foi possível carregar saldos da ADM.", errorSaldos); return; }
    ...
    for (const item of fallbackDoutores) {
      ...
      todosPorNome.set(chave, {
        id: item.id, nome: item.nome, nome_normalizado: chave,
        credito: toNumber(item.credito, 0), pix_key: item.pix_key || "",
        ativo: item.ativo !== false,
        updated_by_email: item.updated_by_email || null,
        updated_by_nome: item.updated_by_nome || null,
        updated_at: null, origem: "dashboard"
      });
    }
    ...
    for (const competencia of meses) {
      ...
      for (const doutor of todosPorNome.values()) {
        ...
        const utilizado = utilizadoRegistros > 0 ? utilizadoRegistros : (utilizadoFallback > 0 ? utilizadoFallback : utilizadoSupabase);
        const creditoFinal = calcularSaldoFinalDoMes(creditoInicial, utilizado);

        baseMes.push({
          doutor_id: doutor.id, doutor: doutor.nome,
          credito_inicial: creditoInicial, utilizado, credito_disponivel: creditoFinal,
          credito_final: creditoFinal, pix_key: doutor.pix_key || original?.pix_key || "",
...
function preencherFiltroDoutor() {
  ...
  const doutores = [...new Set([...nomesRegistros, ...nomesSaldos])].sort((a, b) => a.localeCompare(b, "pt-BR"));
...
function obterStatus(percentual) {
  if (percentual >= 100) return { classe: "status-red",    texto: "Bloqueado",  dot: "dot-red",    tipo: "bloqueado" };
  if (percentual >= 50)  return { classe: "status-yellow", texto: "Atenção",    dot: "dot-yellow", tipo: "atencao"   };
  return                        { classe: "status-green",  texto: "Controlado", dot: "dot-green",  tipo: "controlado" };
}
...
function montarResumoDoutores(saldos) {
  return saldos.map(item => {
    ...
    return { doutor_id: item.doutor_id, doutor: item.doutor, creditoInicial, utilizado, creditoDisponivel, percentual, status };
...
function garantirPainelStatusInicio() {
  ...
  // Inserir ANTES do #secaoPixMes
  const secaoPixMes = byId("secaoPixMes");
  if (secaoPixMes) dashboardView.insertBefore(painel, secaoPixMes);
  else dashboardView.appendChild(painel);
...
function preencherListaCidadesAtuais() {
  ...
  lista.innerHTML = cidades.map(cidade => `<option value="${escapeHtml(cidade)}"></option>`).join("");
}
...
async function carregarCidadesAtuais(competencia) {
  try {
    ...
    const client = validarSupabasePronto();
    const { data, error } = await client.from("doutores_cidade_atual").select("*").eq("competencia", competencia);
    if (error) throw error;
    cidadesAtuaisCache = {};
...
async function salvarCidadeAtualDoutor(doutorNome, cidadeAtual) {
  try {
    ...
    const { error } = await client.from("doutores_cidade_atual").upsert({
      competencia, doutor_nome_normalizado: chave, doutor_nome: doutorNome,
      cidade_atual: cidadeAtual || null, updated_by_email: email,
  ...
  return `
    <div class="doctor-status-card ${tipo}">
      <div class="doctor-card-top">
        ...
        <div class="doctor-card-status ${tipo}">${item.status.texto}</div>
      </div>
...
function renderizarCardsStatusInicio(saldos) {
  ...
  if (elBloqueados)  elBloqueados.innerHTML  = bloqueados.length  ? bloqueados.map(montarCardDoutorStatus).join("")  : `<div class="empty-status-card">Sem doutores bloqueados.</div>`;
  if (elAtencao)     elAtencao.innerHTML     = atencao.length     ? atencao.map(montarCardDoutorStatus).join("")     : `<div class="empty-status-card">Sem doutores em atenção.</div>`;
  if (elControlados) elControlados.innerHTML = controlados.length ? controlados.map(montarCardDoutorStatus).join("") : `<div class="empty-status-card">Sem doutores controlados.</div>`;
}
...
async function atualizarPainelInicioBonito() {
  ...
  renderCards(getRegistrosFiltrados());
...
function exportarCSV() {
  ...
  if (!registros.length) { alert("Não há dados para exportar."); return; }
  const headers = ["competencia","data","cidade","responsavel_fiscal","doutor_final","paciente","valor","valor_descontado","pendente"];
  const rows = registros.map(item => [
    item.competencia ?? "", item.data ?? "", item.unidade ?? "",
    item.responsavel_fiscal_lido ?? "", item.doutor_final ?? "",
    item.paciente ?? "", item.valor ?? 0, item.valor_descontado ?? 0, item.pendente ?? 0
  ]);
  ...
  link.href = url; link.download = `pix_doutores_${competencia}.csv`; link.click();
...
async function carregarDoutoresAdmin() {
  ...
  try {
    const { data: doutoresSupabase } = await client
      .from("doutores_config")
      .select("id, nome, nome_normalizado, credito, pix_key, ativo, updated_by_email, updated_by_nome, updated_at")
      .order("nome", { ascending: true });

    const { data: saldosSupabase } = await client
      .from("doutores_saldos_mensais").select("*").eq("competencia", competencia);
    ...
    // ─── Ordenar: ativos primeiro (A-Z), inativos no final (A-Z)
    ...
    if (!doutores.length) {
      tbody.innerHTML = `<tr><td colspan="11" class="empty-state">Nenhum doutor cadastrado</td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = doutores.map(item => {
      ...
      const responsavelUltimo = saldoSupabase.updated_by_nome || saldoSupabase.updated_by_email || item.updated_by_nome || item.updated_by_email || "";

      // Data da inativação
      const isInativo = item.ativo === false;
      const dataAcao = isInativo && item.updated_at
        ? new Date(item.updated_at).toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit", year: "numeric" })
        : "—";

      return `
        <tr class="${isInativo ? "linha-inativo" : ""}">
          ...
          <td>
            <select data-id="${item.id}" data-field="ativo">
              <option value="true"  ${!isInativo ? "selected" : ""}>Ativo</option>
    ...
    tbody.innerHTML = `<tr><td colspan="11" class="empty-state">Erro ao carregar doutores</td></tr>`;
  }
}
...
async function salvarDoutor(id) {
  try {
    ...
    const payloadDoutor = {
      nome, nome_normalizado: normalizarNome(nome),
      credito: creditoBase, pix_key: pixKey || null, ativo,
      updated_by_email: emailAtual, updated_by_nome: nomeResponsavel
    };

    const doutorIdReal = await garantirDoutorNoSupabase(id, payloadDoutor);
    const { error: errorDoutor } = await client.from("doutores_config").update(payloadDoutor).eq("id", doutorIdReal);
    ...
    const { data: saldoExistente, error: errorBuscaSaldo } = await client
      .from("doutores_saldos_mensais").select("*")
      .eq("competencia", competencia).eq("doutor_id", doutorIdReal).maybeSingle();
    if (errorBuscaSaldo) throw errorBuscaSaldo;

    const payloadSaldo = {
      credito_inicial: creditoInicial, utilizado, credito_final: creditoFinal,
      ajuste_manual: ajusteManual, observacao,
      updated_by_email: emailAtual, updated_by_nome: nomeResponsavel
    };

    if (saldoExistente) {
      const { error: errorSaldo } = await client.from("doutores_saldos_mensais").update(payloadSaldo).eq("id", saldoExistente.id);
      if (errorSaldo) throw errorSaldo;
    } else {
      const { error: errorInsertSaldo } = await client.from("doutores_saldos_mensais").insert({ competencia, doutor_id: doutorIdReal, ...payloadSaldo });
...
async function removerDoutor(id) {
  ...
  try {
    mostrarMensagemAdmin("");
    if (!isUuid(id)) { mostrarMensagemAdmin("Este doutor ainda não existe no Supabase. Nada para excluir.", true); return; }
    const client = validarSupabasePronto();
    const { error: errorSaldo } = await client.from("doutores_saldos_mensais").delete().eq("doutor_id", id);
    if (errorSaldo) throw errorSaldo;
    const { error: errorDoutor } = await client.from("doutores_config").delete().eq("id", id);
    if (errorDoutor) throw errorDoutor;
    await sincronizarSaldosAdminNoDashboard();
    await atualizarDashboard();
    await carregarDoutoresAdmin();
    ...
    console.error("Erro ao remover doutor:", err);
...
async function adicionarDoutor() {
  try {
    ...
    const competencia = getCompetenciaAtual();
    if (!nome) { mostrarMensagemAdmin("Informe o nome do doutor.", true); return; }
    if (Number.isNaN(credito) || credito < 0) { mostrarMensagemAdmin("Informe um crédito válido.", true); return; }
    ...
    const payloadDoutor = {
      nome, nome_normalizado: normalizarNome(nome),
      credito, pix_key: pixKey || null, ativo,
      updated_by_email: emailAtual, updated_by_nome: nomeResponsavel
    };

    const insertDoutor = await client.from("doutores_config").insert(payloadDoutor).select().single();
    if (insertDoutor.error) throw insertDoutor.error;
    const novoDoutor = insertDoutor.data;

    const payloadSaldo = {
      competencia, doutor_id: novoDoutor.id,
      credito_inicial: credito, utilizado: 0, credito_final: credito,
      ajuste_manual: 0, observacao: null,
      updated_by_email: emailAtual, updated_by_nome: nomeResponsavel
    };
    ...
    const mensagem = [err?.message || "Erro desconhecido", err?.details ? `Detalhes: ${err.details}` : "", err?.hint ? `Dica: ${err.hint}` : "", err?.code ? `Código: ${err.code}` : ""].filter(Boolean).join(" | ");
...
async function iniciarAplicacao() {
  try {
    ...
    try {
      ...
      if (byId("tituloDashboard")) byId("tituloDashboard").textContent = "PIX Doutores";
      if (byId("subtituloDashboard")) byId("subtituloDashboard").textContent = "Erro ao carregar os dados da dashboard.";
      if (byId("cardsResumo")) byId("cardsResumo").innerHTML = `<div class="stat-card"><div class="stat-title">Status</div><div class="stat-value">Falha ao carregar data/dashboard_data.json</div></div>`;
      if (byId("tabelaPixMes")) byId("tabelaPixMes").innerHTML = `<tr><td colspan="7" class="empty-state">Erro ao carregar dados</td></tr>`;
    }
  } catch (erro) {
...
byId("loginForm")?.addEventListener("submit", async (event) => {
  ...
  mostrarMensagemAuth("");
  if (!email || !password) { mostrarMensagemAuth("Preencha e-mail e senha.", true); return; }
  try {
    ...
    mostrarDashboard();
    preencherBadgeUsuario();
    try { await carregarDashboardInterno(); } catch (e) { console.error(e); }
  } catch (erro) { console.error(erro); mostrarMensagemAuth(erro.message || "Não foi possível entrar.", true); }
});
...
byId("btnCriarAcesso")?.addEventListener("click", async () => {
  ...
  const password = byId("password")?.value.trim() || "";
  mostrarMensagemAuth("");
  if (!email || !password) { mostrarMensagemAuth("Preencha e-mail e senha para criar o acesso.", true); return; }
  try { await criarAcessoSupabase(email, password); mostrarMensagemAuth("Acesso criado com sucesso. Confira seu e-mail."); }
  catch (erro) { console.error(erro); mostrarMensagemAuth(erro.message || "Não foi possível criar o acesso.", true); }
});

byId("btnForgotPassword")?.addEventListener("click", async () => {
  const email = byId("email")?.value.trim() || "";
  if (!email) { mostrarMensagemAuth("Digite seu e-mail para recuperar a senha.", true); return; }
  try { await enviarRecuperacaoSenha(email); mostrarMensagemAuth("Enviamos um link de recuperação para seu e-mail."); }
  catch (erro) { console.error(erro); mostrarMensagemAuth("Não foi possível enviar o e-mail de recuperação.", true); }
});

byId("btnLogout")?.addEventListener("click", async () => {
  try { await logoutSupabase(); } catch (err) { console.error(err); }
  currentUser = null; currentUserIsAdmin = false; dashboardData = null;
  mostrarTelaLogin();
});

byId("btnTabDashboard")?.addEventListener("click", () => mostrarDashboard());
...
byId("filtroMes")?.addEventListener("change", async () => {
  ...
  await sincronizarSaldosAdminNoDashboard();
  await atualizarDashboard();
  if (!byId("adminView")?.classList.contains("hidden")) await carregarDoutoresAdmin();
});
...
byId("btnLimpar")?.addEventListener("click", async () => {
  ...
  preencherFiltroCidade();
  preencherFiltroDoutor();
  byId("filtroCidade") && (byId("filtroCidade").selectedIndex = 0);
  byId("filtroDoutor") && (byId("filtroDoutor").selectedIndex = 0);
  await sincronizarSaldosAdminNoDashboard();
  await atualizarDashboard();
});
...
if (window.supabaseClient) {
  window.supabaseClient.auth.onAuthStateChange(async (_event, session) => {
    if (!session) { currentUser = null; currentUserIsAdmin = false; mostrarTelaLogin(); return; }
    currentUser = session.user;
  });
}
